...
- Specs: add `src/asr_engine/modules/fake.py` + `tests/modules/test_fake.py` to `fake-module.md` frontmatter and drop its "Frontmatter note"; add `pyproject.toml` + `tests/modules/test_registry.py` to `asr-module-interface.md` frontmatter; status flips (see Verification).
...
5. **`FakeASRModule`** (`modules/fake.py`), per `specs/fake-module.md`:
...
6. **Fast tests.**
   ...
   - `tests/modules/test_fake.py`: the scenarios listed in `specs/fake-module.md` "Testing". Drive `start()` as a task with a hand-fed `asyncio.Queue`, putting exact byte counts to place the clock; record `(clock, transcript, is_final, confidence)` in the callback.
...
Then mark this plan `Done` (here and in [_index.md](_index.md)) and flip to `Implemented` (file + [specs/_index.md](../specs/_index.md)): `asr-module-interface.md`, `deepgram-module.md`, `project.md`, `configuration.md`, `gradio-demo.md`, and `fake-module.md` (Stable → Implemented). `testing.md` stays `Updated` until the follow-up e2e plan is `Done`.
