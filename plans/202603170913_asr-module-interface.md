...
- [x] Implement `ASRResult` dataclass in `modules/base.py`:
  - Fields: `transcript: str`, `is_final: bool`, `confidence: float | None`
- [x] Implement `ResultCallback` type alias: `Callable[[ASRResult], Awaitable[None]]`
- [x] Implement `ASRModule` ABC in `modules/base.py`:
...
- [x] Populate `REGISTRY` dict in `modules/__init__.py`:
  - Initially empty `{}` (Deepgram added in plan 05)
- [x] Implement `load_module(asr_config: dict) -> ASRModule` in `modules/__init__.py`:
