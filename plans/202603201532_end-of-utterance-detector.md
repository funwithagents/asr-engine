...
- [x] Create `src/asr_mcp/end_of_utterance_detector.py`:
...
- [x] Delete `src/asr_mcp/listen_session.py`
- [x] Update `server.py`:
...
- [x] Update `tests/test_listen_session.py` → rename to
...
- [x] Rename `submit_words` → `trigger_words` throughout `asr_to_terminal.py`
  (constructor parameter, instance attribute `_submit_words` → `_trigger_words`,
  internal usages)
- [x] Update `AsrToTerminal.__init__` to accept new parameters:
...
- [x] Add `async _session_loop(self) -> None`:
  - Loop indefinitely:
...
- [x] Update `AsrToTerminal._on_event`:
...
- [x] Rename `--submit-words` → `--trigger-words` in `argparse`
- [x] Add arguments to `argparse` in `asr_to_terminal.main()`:
...
- [x] Pass new args through `_run()` → `AsrToTerminal()`

---

### 4. Update unit tests (`tests/test_asr_to_terminal.py`)

- [x] Update existing tests to use new `AsrToTerminal` constructor signature
  where needed (new params have defaults, so most tests need no change)
- [x] Add test: `trigger_word` mode — submit word in final → session loop fires
  Enter (mock `EndOfUtteranceDetector.wait` to return immediately with
  `end_reason="trigger_word"`)
- [x] Add test: `timeout` mode — session loop fires Enter on EOS timeout (mock
  `EndOfUtteranceDetector.wait` to return with `end_reason="end_of_speech_timeout"`)
- [x] Add test: `start()` launches `_session_loop_task`; `stop()` cancels it
- [x] Verify all existing tests still pass (no behaviour change for the typing
  logic itself)
...
- [x] Add `test_e2e_asr_to_terminal_timeout`:
...
- [x] Add Plan 13 row to `plans/plans.md` (mark as done when complete)
- [x] Add Plan 13 row to `implementation-details/implem.md` (mark as not yet
  written; write the file after implementation)
- [x] Write `implementation-details/13-end-of-utterance-detector.md` after all
  code is done:
