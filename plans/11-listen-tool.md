...
- [x] Add `EngineConfig` dataclass:
  - `auto_start: bool = True`
- [x] Add `ListenConfig` dataclass:
...
- [x] Add `engine: EngineConfig` and `listen: ListenConfig` fields to `AppConfig`
- [x] Parse `engine` block in `load_config`: read `auto_start`, default `True`
- [x] Parse `listen` block in `load_config`:
...
- [x] Unit tests (`tests/test_config.py`):
...
- [x] Create `src/asr_mcp/speech_utils.py` with:
  ```python
  ...
  ```
- [x] Unit tests (`tests/test_speech_utils.py`):
...
- [x] Replace `AsrToTerminal._contains_submit_word` with a call to
  `speech_utils.contains_trigger_word(transcript, self._submit_words)`
- [x] Delete the private method
- [x] Verify existing unit tests in `tests/test_asr_to_terminal.py` still pass
...
- [x] Create `src/asr_mcp/listen_session.py`
- [x] Implement `ListenResult` dataclass:
  - `transcript: str`
  - `end_reason: str`  — `"trigger_word"` | `"end_of_speech_timeout"` | `"initial_silence_timeout"`
- [x] Implement `ListenSession.__init__`:
...
- [x] Implement `async on_result(result: ASRResult) -> None`:
...
- [x] Implement `async wait() -> ListenResult`:
...
- [x] Unit tests (`tests/test_listen_session.py`) — all timers must be injectable/patchable so the
...
- [x] Remove the moved code from `client.py`
- [x] Update the `asr-mcp-client` entry point in `pyproject.toml` to point to
  `asr_mcp.asr_resource_client:main`
- [x] Verify the existing demo client unit tests in `tests/test_client.py` still pass;
...
- [x] In `run_server`: make `await engine.start()` conditional on `config.engine.auto_start`
- [x] Pass `config.listen` into `create_mcp_server`

---

### 8. `listen` tool (`server.py`)

- [x] Add a `_listen_lock: asyncio.Lock` in `create_mcp_server` to prevent concurrent `listen` calls
- [x] Register the `listen` tool:
...
- [x] Update `helpers.py` (`start_mcp_server`) to accept and forward `engine` and `listen`
  config overrides when writing the temp config file
- [x] Add `test_e2e_listen_trigger_word`:
...
- [x] Create `implementation-details/11-listen-tool.md`:
  ...
  - Known limitations
- [x] Update `implementation-details/07-mcp-server.md`:
