...
| 4 | [MCP Server](mcp-server.md) | Resource `asr://result`, tools (`start`, `stop`, `is_running`, `listen`), `auto_start` config, server lifecycle | no |
...
| 10 | [ASR to Terminal](asr-to-terminal.md) | Progressive terminal injection via xdotool/ydotool, submit word detection | yes |
