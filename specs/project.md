...
- **Dev environment has every extra.** Because pyright type-checks the provider modules and the MCP server, and both test tiers exercise them (including the MCP `examples/`), the `dev` dependency group self-references the package with every provider and transport extra (`"asr-engine[deepgram,mcp]"`), so a plain `uv sync` gives contributors a complete venv. Library consumers never pull the dev group. (If a uv version rejects the self-reference, the fallback is documenting `uv sync --all-extras` in AGENTS.md "Commands".)
