...
- **Dev environment has every extra.** Because pyright type-checks the provider modules and the MCP server, and both test tiers exercise them (including the MCP `examples/`), an `all` extra bundles every provider and transport extra (`all = ["asr-engine[deepgram,mcp]"]`, also a public convenience: `pip install 'asr-engine[all]'`), and the `dev` dependency group self-references it (`"asr-engine[all]"`), so a plain `uv sync` gives contributors a complete venv. New provider extras are added to `all`, never to the dev group directly. Library consumers never pull the dev group. (If a uv version rejects the self-reference, the fallback is documenting `uv sync --all-extras` in AGENTS.md "Commands".)
