...
The model runs in-process: no network, no API key. ``KyutaiModule`` holds every
backend-independent behaviour (re-blocking the audio into 80 ms frames, the worker
thread, text accumulation, finalization, step-budget recycling, backpressure,
lifecycle); the model maths lives behind the ``KyutaiBackend`` protocol, with an
MLX implementation (``_kyutai_mlx.py``, Apple Silicon) and a PyTorch one
(``_kyutai_torch.py``). This file imports neither ``moshi_mlx`` nor ``moshi`` —
the backend is resolved by name in ``__init__`` and imported on first ``start()``.

See specs/kyutai-module.md.
"""
...
_BACKEND_IMPLS = {
    "mlx": (
        "moshi_mlx",
        "kyutai-mlx",
        "asr_engine.modules._kyutai_mlx:MlxKyutaiBackend",
    ),
    "torch": (
        "moshi",
        "kyutai-torch",
        "asr_engine.modules._kyutai_torch:TorchKyutaiBackend",
    ),
}
