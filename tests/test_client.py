...
def test_main_default_server():
    ...
    with patch(
        "asr_mcp.asr_resource_client._run_client", new_callable=AsyncMock
    ) as mock_run:
...
def _make_tool_result(payload: dict, is_error: bool = False):
    content_item = TextContent(type="text", text=json.dumps(payload))
    result = MagicMock()
...
async def test_mcp_tool_client_success():
    ...
    with (
        patch("asr_mcp.tool_client.streamable_http_client") as mock_http,
...
async def test_mcp_tool_client_error_raises():
    ...
    error_item = TextContent(
        type="text", text="A listen session is already in progress."
    )
