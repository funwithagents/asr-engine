...
from asr_mcp.subscriber import ResourceSubscriber
...
async def test_start_creates_background_task():
    ...
    with patch("asr_mcp.subscriber.streamable_http_client") as mock_transport, \
         patch("asr_mcp.subscriber.ClientSession"):
...
async def test_on_event_called_on_resource_updated(capsys):
    ...
    with patch("asr_mcp.subscriber.streamable_http_client") as mock_transport, \
         patch("asr_mcp.subscriber.ClientSession", session_cm):
        mock_transport.return_value.__aenter__ = AsyncMock(
            return_value=(MagicMock(), MagicMock(), lambda: None)
        )
        mock_transport.return_value.__aexit__ = AsyncMock(return_value=None)
...
async def test_reconnects_on_error():
    ...
    with patch.object(subscriber, "_connect", side_effect=_flaky_connect), \
         patch("asr_mcp.subscriber.asyncio.sleep", side_effect=lambda _: real_sleep(0)):
